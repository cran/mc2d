...
of the ``healthy'' French population\footnote{Yes, it makes no sense, but it will help us introducing
smoothly the model.}.
...
modGrowth <- function(duration, mTemp, sdTemp,
  ...
  xLm <- N0Lm + (mTemp > TminLm) * murefLm/log(10) * (sdTemp^2 + (mTemp - TminLm)^2) /  ((Tref - TminLm)^2) * realDuration
  xFF <- N0FF + (mTemp > TminFF) * murefFF/log(10) * (sdTemp^2 + (mTemp - TminFF)^2) /  ((Tref - TminFF)^2) * realDuration
...
consoV <- mcstoc(rempiricalD, values = c(10, 12, 19, 20, 30, 34, 40, 50, 60, 67.5, 80, 100, 250), prob = c(11, 1, 1, 29, 12, 1, 41, 4, 4, 1, 4, 1, 1))
@
...
torn <- tornado(Lm2)
torn
tornunc <- tornadounc.mc(Lm2, quant=NULL)
tornunc
@
\texttt{quant=NULL} request an analysis only on the mean risk, and not on some qunatiles.
