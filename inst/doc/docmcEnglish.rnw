...
%\VignetteIndexEntry{Tools for Two-Dimensional Monte-Carlo Simulations}
...
à 2 Dimensions}). This package provides:
\begin{itemize}
...
purposes, they have been moved to a separate package called \texttt{fitdistrplus}.
...
\[ R\left|a,b\right.\sim N\left(a,b\right) \]
...
dimensional Monte-Carlo simulation, with 1,000 iterations. Using the
...
\texttt{nsv}, the size of the variability dimension (here, 1000), 
...
and, finally, the level of output \texttt{outm}, for multivariate parameters.
...
is now uncertain. The median of the 100 simulations leads to a best
estimateof 0.0445, with a 95\% credible
...
distribution is chosen for each parent parameter. For developing
a parametric distribution from data, the \texttt{fitdistrplus}
...
An \texttt{mcnode} is an array of dimension \texttt{nsv} * \texttt{nsu} * \texttt{nvariates}
...
The \texttt{mcstoc} function \footnote{from \texttt{stoc}hastic)} is written as
\footnote{as is standard in R, most arguments have default values and
...
\footnote{Note that the resulting correlation (around 0.4) is obviously an approximation
...
\noindent \begin{flushleft}
\texttt{plot (x, prec = 0.01, stat = c(median,
mean), lim = c(0.025,0.25, 0.75, 0.975), na.rm =
TRUE, griddim = NULL, xlab = NULL, ylab = Fn(x),
main = , draw = TRUE, ...)}
\par\end{flushleft}
...
Here is a (silly) example building a linear model (in fact 100
...
limit. The recycling rule says that \texttt{c(1, 2, 3)} will be used
in the first dimension, i.e. the variability dimension. Use instead:
...
ndvar(1001);ndunc(101)
mcstoc(rempiricalD,type="V",values=inca)
@

but we will use the \texttt{fitdistrplus}
...
%
\begin{figure}
\caption{\label{fig:Function-mledist}Graph from the \texttt{fitdist} function. }
...
Not so bad (See Figure \ref{fig:Function-mledist}), and better than
...
using the \texttt{bootdist} function of the \texttt{fitdistrplus}
...
two-dimensional simulations. Nevertheless, \texttt{mc2d} is still under 
development: \emph{please report any bugs you get to rpouillot@yahoo.fr.} 
